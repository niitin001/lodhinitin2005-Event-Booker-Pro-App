...
router.post("/auth/register",async(req,res)=>{
  try{
    ...
    return res.status(201).json({user,token:token({id:user.id,role:user.role})});
  }catch(e){req.log?.error(e);return res.status(500).json({message:"Registration failed"});}
});
router.post("/auth/login",async(req,res)=>{
  try{
    ...
    return res.json({user:{id:user.id,name:user.name,email:user.email,role:user.role},token:token({id:user.id,role:user.role})});
  }catch(e){req.log?.error(e);return res.status(500).json({message:"Login failed"});}
});
...
router.get("/providers",async(req,res)=>{
  ...
  }catch{return res.status(500).json({message:"Could not load providers"});}
});
...
router.post("/providers",auth,role("provider","admin"),async(req:any,res)=>{
  ...
  }catch(e){req.log?.error(e);return res.status(500).json({message:"Provider creation failed"});}
});
...
router.post("/bookings",auth,async(req:any,res)=>{
  ...
  }catch(e){req.log?.error(e);return res.status(500).json({message:"Booking failed"});}
});
